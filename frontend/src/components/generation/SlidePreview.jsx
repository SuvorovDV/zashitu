      ...
      <div className="w-full max-w-3xl">
        ...
        <ol className="flex flex-col gap-4">
          {Array.from({ length: previewCount }, (_, i) => (
            <li
              key={i + 1}
              className="relative rounded-xl overflow-hidden border border-[#2E2820] bg-[#0F0E0B] shadow-lg shadow-black/30"
            >
              <span
                aria-hidden="true"
      ...
      <ol className="flex flex-col gap-3">
        {plan.slides.map((slide, i) => (
          <li key={i} className="card rounded-xl p-4 flex gap-4">
            <span
              aria-hidden="true"
              className="shrink-0 w-8 h-8 rounded-lg bg-[#221E17] border border-[#4A402F] text-[#7A7362] text-xs font-semibold flex items-center justify-center tabular-nums"
            >
              {i + 1}
            </span>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-2 flex-wrap">
                ...
                <span className="text-[10px] uppercase tracking-wider text-[#7A7362] bg-[#221E17] px-1.5 py-0.5 rounded">
                  {slide.layout}
                </span>
              </div>
...
function PreviewHeader({ plan, previewCount }) {
  ...
  return (
    <div className="flex items-center justify-between text-xs text-[#7A7362] mb-3 px-1">
      <span>
...
function SlideBody({ slide }) {
  ...
  if (slide.layout === 'callout') {
    return (
      <div className="flex flex-col gap-2">
        {slide.callout && (
          <blockquote className="border-l-2 border-brand-500 pl-3 text-[#F5F1E8] text-sm italic">
            {slide.callout}
          </blockquote>
        )}
        {slide.bullets?.length > 0 && (
          <ul className="list-disc list-inside text-[#B8AE97] text-sm space-y-1">
    ...
    return (
      <div className="grid sm:grid-cols-2 gap-3">
        {(slide.columns || []).map((col, i) => (
          <div key={i} className="bg-[#0F0E0B] rounded-lg p-3 border border-[#2E2820]">
            {col.heading && <p className="text-white text-xs font-semibold mb-1.5">{col.heading}</p>}
            <ul className="list-disc list-inside text-[#B8AE97] text-xs space-y-1">
  ...
  return (
    <ul className="list-disc list-inside text-[#B8AE97] text-sm space-y-1">
      {(slide.bullets || []).map((b, i) => <li key={i}>{b}</li>)}
      {slide.source_ref && (
        <li className="list-none text-[11px] text-[#7A7362] italic mt-1.5">{slide.source_ref}</li>
      )}
    </ul>
  )
}
