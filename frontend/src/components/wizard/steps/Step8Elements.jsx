          ...
          return (
            <label
              ...
              className={`flex items-center gap-3 px-4 py-3 rounded-xl border cursor-pointer transition-colors select-none ${
                checked ? 'border-brand-500 bg-brand-500/15' : 'card hover:border-[#4B4A42]'
              }`}
            >
              <div className={`w-4 h-4 rounded flex items-center justify-center flex-shrink-0 border transition-colors ${
                checked ? 'bg-brand-500 border-brand-500' : 'border-[#4B4A42]'
              }`}>
                {checked && (
              ...
              <span className={`text-sm font-medium ${checked ? 'text-brand-300' : 'text-[#D2CFC1]'}`}>{el}</span>
            </label>
          )
        })}
      </div>
