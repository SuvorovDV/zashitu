...
def _build_slides_prompts(order, skeleton: list):
    ...
    if order.include_speech and order.speech_approved and order.speech_text:
        speech_context = (
            "\n\nУтверждённый пользователем текст выступления:\n"
            f"{order.speech_text[:8000]}\n\n"
            "КРИТИЧНО: слайды ДОЛЖНЫ основываться ТОЛЬКО на этом тексте. "
            "Не добавляй фактов, цифр, примеров или тезисов, которых нет в тексте — "
            "только сжатие и визуальная структура уже сказанного."
        )
