...
def _build_slides_prompts(order, skeleton: list):
    ...
    if allow_enhance:
        enhance_rule = (
            ...
            "1. Перед написанием каждого слайда ЧЕСТНО ответь себе: содержится ли ЯВНО эта конкретная "
            "формулировка/факт/пример в РЕЧИ? (не «близкая тема», а именно этот тезис)\n"
            "2. Если ДА — source_ref обязан указывать место в РЕЧИ: «с. N», «Раздел \"…\"», "
            "«<внешний источник из РЕЧИ>, с. N».\n"
            "3. Если НЕТ (ты дополнил собой) — source_ref ОБЯЗАН быть буквально «общее знание». "
            "Без вариаций. «источник: загруженная работа» здесь ЗАПРЕЩЕНО — это лёгкий fallback "
            "для случаев «факт из работы, но страница не определена», не для случаев «дополнил».\n"
            "4. Ложная атрибуция (поставить «источник: загруженная работа» на дополненный слайд) "
            "— главная ошибка, подрывающая продуктовое обещание. Читатель не сможет отличить факты "
            "из работы от добавленных. Это хуже чем вообще не дополнять.\n"
            "5. Смешивание в одном буллете тезисов из РЕЧИ и «общих знаний» запрещено — "
            "разводи на разные слайды или оставь только из РЕЧИ.\n"
            "\n"
            "Признаки слайда-кандидата на «общее знание» (чаще встречается, чем кажется):\n"
            "- Список конкретных примеров (профессии, технологии, кейсы), которых нет в РЕЧИ\n"
            "- Статистические утверждения («большинство», «значительная часть») без цифр из РЕЧИ\n"
            "- Упоминание процессов, не описанных в РЕЧИ (например, «радиологи конкурируют с ИИ»)\n"
            "- Аналогия или сравнение с другой областью, не упомянутой в РЕЧИ"
        )
        fact_integrity_mode = (
            "Fact integrity (enhance-режим): цифры/имена/даты/примеры из РЕЧИ — воспроизводи точно. "
            ...
            "выдуманных ФИО и специфических процентов без опоры на авторитетные источники."
        )
    else:
        enhance_rule = (
