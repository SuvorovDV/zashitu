...
def compose_skeleton_system_prompt(
    *, persona_block: str, structural_canon_block: str, n_slides: int,
    detail_level: str, allow_images: bool,
) -> str:
    """Собирает type-aware промт для skeleton-derivation. Каждый тип презентации
    даёт свой структурный канон — школьник не получит «Цели и задачи»,
    деловой доклад не получит «Что такое X»."""
    images_note = (
        "Крупные image-слайды не используй (image_full/image_side)."
...
чьи заголовки соответствуют логическим шагам речи в том же порядке.

## СТРУКТУРНЫЙ КАНОН ДЛЯ ЭТОГО ТИПА ПРЕЗЕНТАЦИИ
...
3. Каждая markdown-таблица в речи → один слайд с layout=table.
4. Каждый числовой ряд по времени/категориям → layout=chart.
5. Выразительные цитаты в кавычках → layout=quote (1-2 на колоду максимум).
6. Плотность: detail={detail_level} — при brief дольше каждый слайд, при detailed — дробнее.
7. Заголовки слайдов — короткие (≤60 символов), без номеров, отражают конкретный фрагмент речи.
8. НЕ выдумывай темы, которых нет в речи. НЕ добавляй слайды-заглушки типа «Литература», «Цели и задачи», «Методология»,
   если их нет в самой речи и они не органичны для канона типа презентации (см. выше).
9. Следуй структурному канону типа: для школьного реферата — Что/Почему/Как/Примеры/Выводы;
   для делового доклада — Контекст/Идея/Доказательства/Кейсы/Вывод; для академзащиты — Введение/Методы/Результаты/Выводы.
10. Если речь короткая — сгруппируй иначе, но {n_slides} ровно.
